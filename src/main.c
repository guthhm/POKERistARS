...
int main(int argc, char **argv) {
    ...
    int loop = 0;
    int *input_int;
    char *player_hands, *char_res;
    char input[10][3], splited_hands[1][7][3], temp_hand[7][3] = {0};
    char args[8][3] = {"-c", "-d1", "-d2", "-d3", "-d4", "-dx", "-s1", "-o"};
    HAND hand_input[2] = {0}, hand_result[2];

    loop = YES;
    for (int mode = 0; mode < 9; mode++)     // for para deteção do modo de execução do programa
    {
                ...
                case 0:  // Modo linha de comando
                    ...
                    switch (argc)
                    {
                        ...
                        case 11:
                            ...
                            {
                                for (int k = 0; k < 5; k++)
                                    sprintf(temp_hand, "%d%c ", hand_result[i].card_values[hand_result[i].best_combination[k]], hand_result[i].card_naipes[hand_result[i].best_combination[k]]);
